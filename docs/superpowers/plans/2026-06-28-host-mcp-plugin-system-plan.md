...
>
> **Carried-over items from the Phase 3 OAuth security review (apply immediately after Phase 4, before Phase 5 — security-critical):**
> - **OAuth-M1 (MAJOR) — no HTTPS/SSRF guard in the OAuth discovery/token path.** `McpOAuthManager.DiscoverEndpointsAsync` follows server-controlled URLs (resource_metadata from `WWW-Authenticate`, `authorization_servers[0]`, AS-metadata well-knowns, `authorization_endpoint`/`token_endpoint`) and fetches/POSTs to them with NO scheme check or private-IP block → blind SSRF (e.g. `http://169.254.169.254/…`, internal ports) AND exfiltration of `code`+`code_verifier`+`client_id`+`client_secret` to a plaintext/attacker `token_endpoint`. Also `McpServerConnectionFactory.CreateHttp` does NOT call `McpUrlSecurity.IsInsecureForCredentials` on the OAuth bearer path (the apiKey path does). FIX: enforce `https` (allow loopback http only if intentionally supporting a local AS) on every discovered URL before any fetch/POST; add an SSRF block for private/link-local/loopback ranges on the discovery fetches; call `McpUrlSecurity.IsInsecureForCredentials(server.Url)` in `CreateHttp` for the OAuth path too. Unit-test the guards.
> - **OAuth-m1 (minor) — `JsonException.Message` logged from a 2xx-but-malformed token response** (`McpOAuthManager` CompleteAsync/RefreshAccessTokenAsync) could include a body snippet (token). Log a fixed `"malformed token response"` for the JSON case.
> - **OAuth-m2 (minor) — abandoned pending-authorization entries** (holding the PKCE verifier + client_secret in memory) are never swept until process exit. Sweep past-TTL entries opportunistically in `BuildAuthorizationUrlAsync` (or a periodic sweep).
